...
async function seedUser () {
  try {
    let user = await User.findOne({
      'username':'Reinhold_Morar',
      'isDeleted':false,
      'isActive':true
    });
    if (!user || !user.isPasswordMatch('fthCBDDR_OQD8zm') ) {
      let user = new User({
        'password':'fthCBDDR_OQD8zm',
        'username':'Reinhold_Morar',
    ...
    let admin = await User.findOne({
      'username':'Demetrius.Feil',
      'isDeleted':false,
      'isActive':true
    });
    if (!admin || !admin.isPasswordMatch('9PgoVdUAClkcfDL') ) {
      let admin = new User({
        'password':'9PgoVdUAClkcfDL',
        'username':'Demetrius.Feil',
...
async function seedUserRole (){
  try {
    ...
    if (user && user.isPasswordMatch('fthCBDDR_OQD8zm') && userRole){
    ...
    if (admin && admin.isPasswordMatch('9PgoVdUAClkcfDL') && adminRole){
      let count = await UserRole.countDocuments({
        userId: admin.id,
        roleId: adminRole.id
      });
